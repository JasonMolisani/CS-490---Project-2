...
8. When setting up the tests, I had difficulties importing my bot file, which was one folder up. Checking slack showed I was not the first to encounter this problem. the solution posted there worked, but only to run the tests from within the tests folder. Circle CI tries to run them from the project folder. Looking at the solution I was given, I was able to adapt it to add the current folder to the search path (the solution added the parent folder of the $PWD that launched the file). By adding the current folder, the imports for `mocked_unit_tests.py` and `unmocked_unit_tests.py` should now work when run from within either the root folder or any of the first level sub-folders.
