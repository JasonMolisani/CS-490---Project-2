...
class Bot:
    ...
    def funtranslate(self, cmdArgs):
        ...
        response = requests.get(
            "https://api.funtranslations.com/translate/pirate.json", params=parameters
        )
    ...
    def joke(self):
        ...
        header = {
            "user-agent": "Student Chat App (author: https://github.com/JasonMolisani)",
    ...
    @staticmethod
    def isImageURL(validURL):
        if not Bot.isValidURL(validURL):
            return False
    ...
    @staticmethod
    def adjustMessageHTML(rawMessage):
        words = rawMessage.split()
        processedMessage = ""
        for word in words:
            try:
                if Bot.isValidURL(word):
                    if Bot.isImageURL(word):
